...
function App() {
  ...
  const [modelStatus, setModelStatus] = useState<"none" | "loading" | "loaded">(
    "none"
  );
  ...
  const [selectedUploadedFileName, setSelectedUploadedFileName] =
    useState<string>("");
  const [currentUploadSessionFiles, setCurrentUploadSessionFiles] = useState<
    string[]
  >([]);
  const [isInferring, setIsInferring] = useState<boolean>(false);
  const [realtimeOverlayFrame, setRealtimeOverlayFrame] = useState<
    string | null
  >(null);
  const [isAnalysisVideoSelectOpen, setIsAnalysisVideoSelectOpen] =
    useState(false);
  const [inferenceMode, setInferenceMode] = useState<"default" | "AR" | "AL">(
    "default"
  );
  ...
  useEffect(() => {
    if (uploadedFiles.length === 0) {
      ...
      !uploadedFiles.some((f) => f && f.name === selectedUploadedFileName)
    ) {
  ...
  useEffect(() => {
    if (Object.keys(annotationData).length > 0) {
      ...
      const uploadedVideoNames = uploadedFiles
        .filter((f) => f && f.name)
        .map((f) => f.name)
        .sort();
      const isSameLength =
        annotationVideoNames.length === uploadedVideoNames.length;
      const isSameNames = annotationVideoNames.every(
        (name, idx) => name === uploadedVideoNames[idx]
      );
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filteredFiles = Array.from(files).filter(
      (file) =>
        ...
        !alreadyUploadedNames.has(file.name)
    );
    ...
    for (let i = 0; i < filteredFiles.length; i++) {
      ...
      try {
        ...
        if (!uploadRes.ok)
          throw new Error(`파일 업로드 실패: ${file && file.name}`);
        const data = await uploadRes.json();
        setUploadedFiles((prev) => {
          const newFiles = [
            ...prev,
            ...(Array.isArray(data.files)
              ? data.files.filter((f: UploadedFile) => f && f.name)
              : []),
          ];
          return newFiles.filter(
            (v, idx, a) =>
              v && v.name && a.findIndex((t) => t && t.name === v.name) === idx
          );
        });
        // 실제 업로드된 파일만 카운트
  ...
  const handleRemoveFile = async (fileName: string) => {
    const res = await fetch(`/upload/${encodeURIComponent(fileName)}`, {
      method: "DELETE",
    });
    if (res.ok) {
      setUploadedFiles((prev) =>
        prev.filter((f: UploadedFile) => f && f.name && f.name !== fileName)
      );
          ...
          if (file.name.endsWith(".txt")) {
            ...
            lines.forEach((line) => {
              ...
              if (inferenceMode === "AR") {
                if (parts.length !== 2) {
                  throw new Error(
                    `AR 모드에서는 비디오 이름과 레이블만 필요합니다: ${line}`
                  );
                }
                ...
                if (label.toLowerCase().includes("non")) {
                  if (parts.length !== 2) {
                    throw new Error(
                      `NonFight 레이블에는 프레임 번호가 필요하지 않습니다: ${line}`
                    );
                  }
                  parsedData[videoName]["AL"] = [];
                } else {
                  if (parts.length < 4) {
                    throw new Error(
                      `Fight 레이블에는 최소 하나의 시작/종료 프레임 쌍이 필요합니다: ${line}`
                    );
                  }
  ...
  const renderModelLoader = () => {
    switch (modelStatus) {
      case "loading":
        return <ModelLoadingAnimation isLoading={true} />;
      case "loaded":
        return (
      ...
      default:
        return <ModelSelector onSubmit={handleLoadModel} />;
    }
  };

  const isAnalysisComplete =
    !!inferenceState &&
    Array.isArray(inferenceState.events) &&
    inferenceState.events.some(
      (ev: { type: string }) => ev.type === "complete"
    );
  ...
  const handleOpenAnalysisVideoSelect = async () => {
    try {
      ...
      alert(
        "분석 비디오 로딩에 실패했습니다. 백엔드 서버 로그를 확인해 주세요."
      );
    }
  };
  ...
  const selectedVideoUrl = selectedVideo
    ? `http://localhost:10000/video/${selectedVideo.replace(
        "_overlay.mp4",
        ""
      )}/overlay`
    : "";
  ...
  const handleNasFolderUpload = async () => {
    ...
    if (
      !(
        trimmedPath.startsWith(nasTargetPath) ||
      ...
      alert(
        `경로 형식이 올바르지 않습니다. 절대 경로(${nasTargetPath}) 또는 상대 경로(./, ../)를 사용하세요.`
      );
      return;
    }
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      >
            ...
            >
              ...
              {totalFilesToUpload > 0 ? (
                <Typography
                  ...
                  sx={{ mb: 1 }}
                >
                  업로드 중: {uploadedFileCount} / {totalFilesToUpload} (
                  {((uploadedFileCount / totalFilesToUpload) * 100).toFixed(0)}
                  %)
                </Typography>
              ) : (
                <Typography
                  ...
                  {showNasProgress && (
                    <Box sx={{ width: "100%", mt: 1 }}>
                      <LinearProgress
                        variant="determinate"
                        value={
                          nasCopyProgress.total_files > 0
                            ? (nasCopyProgress.copied_files /
                                nasCopyProgress.total_files) *
                              100
                            : 0
                        }
                      ...
                      >
                        <Typography variant="body2">
                          복사 진행률: {nasCopyProgress.copied_files} /{" "}
                          {nasCopyProgress.total_files}
                        </Typography>
                        <Button
                          variant="outlined"
              ...
              <FormControl fullWidth size="small" sx={{ mt: 1 }}>
                <InputLabel id="uploaded-video-select-label">
                  업로드된 비디오
                </InputLabel>
                <Select
                ...
                >
                  {uploadedFiles
                    .filter((f: UploadedFile) => f && f.name)
                    .map((f: UploadedFile) => (
              ...
              <Grid container spacing={1} sx={{ mb: 1 }}>
                <Grid item xs={4}>
                  <TextField
                    ...
                    size="small"
                    fullWidth
                    sx={{ fontSize: "0.7rem", input: { textAlign: "center" } }}
                  />
                </Grid>
                <Grid item xs={4}>
                  <TextField
                ...
                >
                  ...
                  >
                    ...
                    >
                      {selectedResultFile ? (
                        <VideoPlayer
                          videoUrl={URL.createObjectURL(selectedResultFile)}
                        />
                      ) : (
                        <Box
                          sx={{
                        ...
                        >
                          ...
                          <Typography
                            ...
                            sx={{ mt: 1 }}
                          >
                            ※ NAS(aivanas) save_results 폴더 내의 mp4 파일만
                            선택해 주세요.
                          </Typography>
                        </Box>
                      )}
        ...
        <Dialog
          open={annotationAlertOpen}
          onClose={() => setAnnotationAlertOpen(false)}
        >
          <DialogTitle>어노테이션/비디오 불일치</DialogTitle>
          <DialogContent>
            <Typography>
              어노테이션 파일의 비디오 목록과 업로드된 비디오 목록이 일치하지
              않습니다!
            </Typography>
          </DialogContent>
          <DialogActions>
        ...
        <Dialog
          open={annotationSuccessOpen}
          onClose={() => setAnnotationSuccessOpen(false)}
        >
          <DialogTitle>어노테이션 성공</DialogTitle>
          <DialogContent>
            <Typography>
              어노테이션 파일이 성공적으로 로드되었습니다.
            </Typography>
          </DialogContent>
          <DialogActions>
            <Button onClick={() => setAnnotationSuccessOpen(false)}>
              확인
            </Button>
          </DialogActions>
        </Dialog>
